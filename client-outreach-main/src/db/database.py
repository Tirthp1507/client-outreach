...
class Database:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS contacts (
                    ...
                    whatsapp_link TEXT,
                    source_url TEXT,
                    source_type TEXT NOT NULL DEFAULT 'website_contact',
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_contacts_business ON contacts(business_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_contacts_email ON contacts(email)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_contacts_verif ON contacts(verification_status)")
    ...
    def _row_to_contact(row: sqlite3.Row) -> ContactRecord:
        ...
        return ContactRecord(
            ...
            source_url=d.get("source_url"),
            source_type=ContactSourceType(d.get("source_type") or "website_contact"),
            confidence=ContactConfidence(d.get("confidence") or "medium"),
            verification_status=EmailVerificationStatus(d.get("verification_status") or "unverified"),
            verification_details=json.loads(d.get("verification_details_json") or "{}"),
            is_primary=bool(d.get("is_primary", 1)),
            discovered_at=d["discovered_at"],
        )

    # -------------------------------------------------------------------------
    # Contacts Management & Provenance
    # -------------------------------------------------------------------------

    def save_contact(self, contact: ContactRecord) -> ContactRecord:
        """Insert or replace a discovered contact record with provenance and verification."""
        with self._get_connection() as conn:
            conn.execute(
                """
                INSERT INTO contacts (
                    id, business_id, email, contact_name, role_title, phone,
                    whatsapp_link, source_url, source_type, confidence,
                    verification_status, verification_details_json, is_primary, discovered_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(id) DO UPDATE SET
                    email=excluded.email,
                ...
                (
                    ...
                    contact.source_url,
                    contact.source_type.value if isinstance(contact.source_type, ContactSourceType) else contact.source_type,
                    contact.confidence.value if isinstance(contact.confidence, ContactConfidence) else contact.confidence,
                    contact.verification_status.value if isinstance(contact.verification_status, EmailVerificationStatus) else contact.verification_status,
                    json.dumps(contact.verification_details),
    ...
    def get_contact(self, contact_id: str) -> Optional[ContactRecord]:
        """Fetch a contact record by ID."""
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT * FROM contacts WHERE id = ?", (contact_id,))
            row = cursor.fetchone()
            return self._row_to_contact(row) if row else None

    def get_primary_contact_for_business(self, business_id: str) -> Optional[ContactRecord]:
        """Fetch the primary verified or highest-confidence contact for a business."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
    ...
    def list_contacts_for_business(self, business_id: str) -> List[ContactRecord]:
        """List all discovered contact points for a business."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT * FROM contacts WHERE business_id = ? ORDER BY is_primary DESC, discovered_at ASC",
                (business_id,),
            )
    ...
    def update_contact_verification(
        ...
        """Update the verification status and diagnostic details of a contact."""
        with self._get_connection() as conn:
            conn.execute(
                """
                UPDATE contacts
                ...
                """,
                (
                    status.value if isinstance(status, EmailVerificationStatus) else status,
